...
//        return -1;
    ...
    if (next_step_column[calculate_uni_dimension_index(row, col)] == UNDEFINED_VALUE)
    {
        ...
        next_step_column[calculate_uni_dimension_index(row, col)] = lowest_col;
    }

    return
            calculate_energy(input_pixels, row, col, next_row, next_step_column[calculate_uni_dimension_index(row, col)])
            +
            find_path(input_pixels, energy_map, next_step_column, next_row, next_step_column[calculate_uni_dimension_index(row, col)]);
}


void
eliminate(vector<int>& input_pixels, vector<int>& energy_map, vector<int>& next_step_column, int col) {
    ...
    for (int row = 0; row < HEIGHT; row++) {
        ...
        col = next_step_column[calculate_uni_dimension_index(row, col)];

        next_step_column.erase(
                next_step_column.begin() + index_to_remove
        );
    }
}
...
{
    vector<int> next_step_column = vector<int>(HEIGHT * WIDTH, UNDEFINED_VALUE);
